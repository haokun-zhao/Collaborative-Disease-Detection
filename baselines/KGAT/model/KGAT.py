...
class KGAT(nn.Module):
    ...
    def __init__(self, args,
        ...
        self.A_in = nn.Parameter(torch.sparse_coo_tensor(torch.Size([self.n_users + self.n_entities, self.n_users + self.n_entities]), dtype=torch.float32))
        if A_in is not None:
            self.A_in.data = A_in
    ...
    def update_attention(self, h_list, t_list, r_list, relations):
        ...
        A_in = torch.sparse_coo_tensor(indices, values, torch.Size(shape), dtype=torch.float32)
